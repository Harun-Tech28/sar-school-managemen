...
export const AttendanceMonitor = ({ studentId }: AttendanceMonitorProps) => {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      if (consecutiveAbsences >= 3) {
        newAlerts.push(`⚠️ ${consecutiveAbsences} absences in the last 5 school days`)
      }

      // Check for low attendance
      if (percentage < 75 && total > 10) {
        newAlerts.push(`⚠️ Attendance rate (${Math.round(percentage)}%) is below recommended 75%`)
      }

      // Check for excessive lateness
      if (late > 5 && total > 20) {
        newAlerts.push(`⚠️ ${late} late arrivals recorded`)
      }
      ...
      {alerts.length > 0 && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">Attendance Alerts</h3>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-500 mt-2">
          {stats.percentage >= 90 ? 'Excellent attendance!' :
           ...
           stats.percentage >= 60 ? 'Fair attendance - improvement needed' :
